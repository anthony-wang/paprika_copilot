...
3. **Ask before guessing** when the recipe name, an ingredient quantity/item, a direction step, or servings is missing/unreadable. Don't invent content. Other metadata may be left blank without asking. Max 3 questions per message.
...
Use these exact keys. Omit any key whose value is unknown — do not emit empty strings. `name`, `ingredients`, `directions` are required. Never emit `uid`, `hash`, `photo_hash`, `photo_data`, `photo`, `photo_large`, `photos`, `image_url`, `created` — Paprika sets these on import.
